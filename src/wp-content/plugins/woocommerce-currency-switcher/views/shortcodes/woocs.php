...
//***
        ...
        <form method="<?php echo apply_filters('woocs_form_method', 'post') ?>" action="" class="woocommerce-currency-switcher-form <?php if ($show_flags): ?>woocs_show_flags<?php endif; ?>" data-ver="<?php echo WOOCS_VERSION ?>">
            ...
            <select name="woocommerce-currency-switcher" data-width="<?php echo $width ?>" data-flag-position="<?php echo $flag_position ?>" class="woocommerce-currency-switcher <?php echo $css_class ?>" onchange="woocs_redirect(this.value); void(0);">
    ...
    if (intval($sd_settings) !== -1) {
        ...
        if (isset($_GET['action']) AND $_GET['action'] === 'elementor') {
            ...
            ?>
            <div data-woocs-sd='<?php echo json_encode($sd_settings) ?>' data-woocs-ver='<?php echo WOOCS_VERSION ?>' style="width: <?php echo (isset($sd_settings['width']) ? $sd_settings['width'] . 'px' : 'auto') ?>; max-width: 100%;" data-woocs-sd-currencies='<?php echo json_encode($currencies) ?>'><div class="woocs-lds-ellipsis"><div></div><div></div><div></div><div></div></div></div>
            <?php
        }
    }
}
